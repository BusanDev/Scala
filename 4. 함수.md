...
> 스칼라 개발자는 순수함수를 배타적으로 사용하는 방법을 알아내려고 노력하는 대신, 일반적으로 어느정도 수준에서 절충하고 순수하지 않은 삼수의 수를 줄이는 방법을 강구한다. 
>
...
프로시저에 대해 명시작으로 타입을 Unit으로 지정하면, 코드를 읽는 사용자에게 반환값이 없다는 것을 분명하게 알 수 있다.
...
메소드 내에 반복되어야 할 필요가 있는 로직을 가지고 있으나, 외부 메소드로 주정되어 득이 되지 않는 때가 있다. 이런 경우에 다른 함수 내부에 존재하는 내부 함수를 정의하여 그 함수 내에서만 사용되도록 하는 것이 낫다.
...
* 연산자 재정의 : 지나치게 열정적인 갭라자가 덧셈 연산자를 호출자 몰게 완전히 다른 형태의 연산을 수행하는 자신만의 타입으로 재정의할 수도 있다.
