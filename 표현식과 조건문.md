...
etc (책에서는 other라고 했지만 어떤 문자든 넣을 수 있다. )
...
1. s는 무엇인가?
2. other는 어차피 예외 처리문 안에서만 쓰는 변수인데 효용성이 더 뛰어나다는 이점이 있는가?
